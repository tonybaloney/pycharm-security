...
class FlaskDebugModeInspectionTest: SecurityTestTask() {
    ...
    private fun testCodeString(code: String, times: Int = 1){
        val mockHolder = mock<ProblemsHolder> {
            on { registerProblem(any<PsiElement>(), eq(Checks.FlaskDebugModeCheck.toString())) } doAnswer {}
        }
        ...
        ApplicationManager.getApplication().runReadAction {
            ...
            Mockito.verify(mockHolder, Mockito.times(times)).registerProblem(any<PsiElement>(), eq(Checks.FlaskDebugModeCheck.toString()))
        }
    }
}